...
const createProductValidator = [
  ...
  check('priceAfterDiscount').optional().isNumeric()
    ...
    .custom((value, { req }) => {
      if (req.body.price <= value) {
        throw new Error('Product price after discount must be less than the original price')
      }
      return true;
    }),
  check('colors').optional().isArray().withMessage('Product colors must be provided as an array'),
  check('imageCover').notEmpty().withMessage('Product cover image is required'),
  check('category').notEmpty().withMessage('Product category is required, product must belong to category').isMongoId().withMessage('Invalid Cataegory Id Format')
    .custom(async (value) => {
      const isIdExist = await Category.findById(value)
      if (!isIdExist)
        throw new Error(`No category with id ${value}`)
      return true
    }),

  check('subCategories').optional().isMongoId().withMessage('Invalid subCataegories Id Format').isArray().withMessage('Product subcategories must be provided as an array')
    .custom(async (value) => {
      //Check if subCategories exists in DB
      const subs = await SubCategory.find({ _id: { $in: value } })
      if (value.length !== subs.length)
        throw new Error('One or more subcategories do not exist in the database')

      return true
    })
    .custom(async (value, { req }) => {
      const subs = await SubCategory.find({ parentCategory: req.body.category })
      //Check if all subcategories related to the parent category
      const subsOfCategory = []
      subs.forEach(doc => subsOfCategory.push(doc._id.toString()))
      const isAllIdsRelated = value.every(id => subsOfCategory.includes(id))
      if (!isAllIdsRelated) {
        throw new Error('Some subcategories do not belong to the specified parent category')
      }
  ...
  check('ratingsAverage').optional().isNumeric()
    .withMessage('Product ratings average must be a numeric value')
    .isLength({ min: 1, max: 5 }).withMessage('Product ratings average must be between 1 and 5'),
